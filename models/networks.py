...
def get_norm_layer(norm_type='instance'):
    ...
    if norm_type == 'batch':
        ...
        #norm_layer = functools.partial(nn.InstanceNorm2d(64, eps=1e-05, momentum=0.1, affine=False,track_running_stats=True))
    else:
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], use_parallel = True, learn_residual = False, padding_type='reflect', scale_factor=4):
        ...
        self.block3 = nn.Sequential(*model)

    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor) and self.use_parallel:
            # output = nn.parallel.data_parallel(self.model, input, self.gpu_ids)
            output1 = nn.parallel.data_parallel(self.block1, input, self.gpu_ids)
            output2 = nn.parallel.data_parallel(self.block2, output1, self.gpu_ids)
            output = nn.parallel.data_parallel(self.block3, output1 + output2, self.gpu_ids)
        else:
            # output = self.model(input)
            output1 = self.block1(input)
            output2 = self.block2(output1)
            output = self.block3(output1+output2)
        if self.learn_residual:
            output = input + output
            output = torch.clamp(output,min = -1,max = 1)
        return output
